...
export default function OrdersFormContainer({ order }: Props) {
  ...
  const deleteOrder = useMutation({
    ...
    onError: () => {
      toast.error("Error trying to delete Order!");
    },
  });
  ...
  function handleSubmit(event: SyntheticEvent, id: string) {
    ...
    updateOrderStatus.mutate({ id, status: value });
  }

  if (!session) return <td className="py-6 px-1">{order.status}</td>;
  ...
  return (
    <td>
      ...
      >
        ...
        <button type="submit" className="bg-red-400 p-2 rounded-full w-9 h-9">
